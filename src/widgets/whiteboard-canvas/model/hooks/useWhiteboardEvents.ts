...
import { Point, Stroke, TextElement, ToolType, TransformState } from 'entities/whiteboard';
...
import { useCursorTool } from './tools/useCursorTool';
...
export const useWhiteboardEvents = (props: UseWhiteboardEventsProps) => {
    const {
        canvasRef,
        transformRef,
        tool,
    ...
    // was shared across move/end handlers. We passed those to useCursorTool.
    ...
    // Actually, useCursorTool defines its own refs? 
    // Wait, in my implementation of useCursorTool, I asked for them as props.
    ...
    const cursorTool = useCursorTool({
        ...props,
        isResizingRef,
        resizeHandleRef,
    ...
    const handleStart = useCallback((e: React.MouseEvent | React.TouchEvent) => {
        ...
        // useCursorTool handles: Selection Handles (only if tool==cursor), Text Resize Handles (if editingTextId)
        ...
        if (tool === 'pen') {
            ...
            // If it returned false/void (wait, my useCursorTool returns boolean handled?), it means it missed.
            // My useCursorTool returns {handleStart...} 
            // I should modify useCursorTool to return boolean or handle "miss" internally?
            // In my implementation of useCursorTool, handleStart returns `true` if it did something or matched.
